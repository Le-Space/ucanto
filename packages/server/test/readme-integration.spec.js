...
test('README workflow integration with server-as-channel', async () => {
  ...
  assert.ok(
    !result.out.error,
    `Expected no error, got: ${result.out.error?.message}`
  )
  ...
  const storedLink = context.store.get(
    `file:///tmp/${issuerKey.did()}/me/about`
  )
...
test('README delegation example with server-as-channel', async () => {
  ...
  assert.equal(
    result.out.ok.with,
    `file:///tmp/${alice.did()}/friends/${bob.did()}/about`
  )
})
