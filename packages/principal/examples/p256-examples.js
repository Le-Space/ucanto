...
async function signingAndVerification() {
  ...
  console.log(
    `🔄 Cross-verification: ${crossVerification ? 'VALID' : 'INVALID'}`
  )
  ...
  console.log(
    `[X] Wrong message verification: ${
      shouldFail ? 'VALID' : 'INVALID (expected)'
    }`
  )
...
async function keySerialization() {
  ...
  console.log(
    `🔐 Cross-keypair verification: ${
      restoredVerification ? 'VALID' : 'INVALID'
    }`
  )
...
async function ucanDelegation() {
  ...
  const delegation = await Core.delegate({
    ...
    facts: [{ service: 'example-storage' }, { plan: 'premium' }],
  })
...
async function clientInvocations() {
  ...
  console.log(
    `📏 Size: ${(invocation.capabilities[0].nb.size / 1024 / 1024).toFixed(
      2
    )} MB`
  )
...
async function delegationChain() {
  ...
  console.log(
    `📤 Final invocation capability: ${invocation.capabilities[0].can}`
  )
...
async function algorithmComparison() {
  ...
  console.log('-----------------|---------------|----------------')
  console.log(
    `Signature Size   | ${p256Signature.raw.length
      .toString()
      .padStart(11)} bytes | ${ed25519Signature.raw.length
      .toString()
      .padStart(12)} bytes`
  )
  console.log(
    `DID Length       | ${p256Signer
      .did()
      .length.toString()
      .padStart(11)} chars | ${ed25519Signer
      .did()
      .length.toString()
      .padStart(12)} chars`
  )
  console.log(
    `Sign Time        | ${p256SignTime
      .toFixed(2)
      .padStart(9)} ms | ${ed25519SignTime.toFixed(2).padStart(10)} ms`
  )
  console.log(
    `Verify Time      | ${p256VerifyTime
      .toFixed(2)
      .padStart(9)} ms | ${ed25519VerifyTime.toFixed(2).padStart(10)} ms`
  )
  console.log(
    `Algorithm Code   | ${
      '0x' + p256Signer.signatureCode.toString(16).padStart(9)
    } | ${'0x' + ed25519Signer.signatureCode.toString(16).padStart(10)}`
  )

  console.log('\n💡 Use Cases:')
  console.log(
    '   P-256: NIST compliance, enterprise integration, HSM compatibility'
  )
  console.log(
    '   Ed25519: Performance, smaller signatures, modern applications'
  )
}
...
async function main() {
  try {
    ...
    console.log(
      '   • P-256 Standard: https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.186-4.pdf'
    )
  } catch (error) {
